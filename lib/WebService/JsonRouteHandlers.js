...
var NetFlowRetrieval = require('../NetFlowRetrieval');
var config = require('config');

function JsonRouteHandlers(es, logger) {
    this.es = es;
    this.logger = logger;
...
JsonRouteHandlers.prototype.getFlowsForLast = function(duration, scale, callback) {
    ...
    this.nfRetrieval.getRawFlows(current - (duration_seconds * 1000), current, function(err, response, status) {
