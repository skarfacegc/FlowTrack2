...
NetFlowStorage.prototype.storeFlow = function storeFlow(flow, demo) {
    ...
    if (process.env.NODE_ENV === 'demo' || demo === true) {
        flow.ipv4_src_addr = flow.ipv4_src_addr.replace(/\d+.\d+.\d+.(\d+)/, '10.0.0.$1');
        flow.ipv4_dst_addr = flow.ipv4_dst_addr.replace(/\d+.\d+.\d+.(\d+)/, '10.1.0.$1');
        flow.ipv4_next_hop = flow.ipv4_next_hop.replace(/\d+.\d+.\d+.(\d+)/, '10.2.0.$1');
    }
