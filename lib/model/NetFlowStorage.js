...
NetFlowStorage.prototype.createTemplate = function (callback) {
    ...
    function (error, response, status) {
        if (error) {
            ...
            throw new Error('Template creation failed: ' + error);
        } else {
...
// Refresh indices
NetFlowStorage.prototype.refreshIndices = function () {
    var indexNamePattern = this.index_name + '*';

    this.client.indices.refresh({index: indexNamePattern}, function (err, resp, status) {
        if (err) {
